...
//hapus data
     ...
     if ($result){//kirim ke database
        ...
        saveActivity($connect, $userId, 'delete', "Menghapus data Galeri ID $id");
        //kalau berhasil kembali ke halamn index
        echo "
        <script>alert('Data Berhasil dihapus'); window.location.href = '../../pages/galleries/index.php';
        </script>
        ";
        //kalau gagal kembali ke halaman create
        }else{
            echo"
            <script>alert(Data Gagal dihapus); window.location.href = '../../pages/galleries/create.php';
            </script>
            ";
